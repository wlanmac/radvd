	...
	while (len > 0) {
		...
		case ND_OPT_DNSSL_INFORMATION:
			...
			for (offset = 0; offset < (dnsslinfo->nd_opt_dnssli_len - 1) * 8;) {
				 ...
				 */
				if ((sizeof(suffix) - strlen(suffix)) < (label_len + 2) || label_len > label_len + 2 || &dnsslinfo->nd_opt_dnssli_suffixes[offset + label_len]
				    - opt_str >= len || offset + label_len < offset) {
				...
				if (suffix[0] != '\0')
					strcat(suffix, ".");
				strncat(suffix, (char *)
					&dnsslinfo->nd_opt_dnssli_suffixes[offset], label_len);
				offset += label_len;
			}
			break;
		default:
